...
def lib2to3_parse(src_txt: str, target_versions: Collection[TargetVersion] = ()) -> Node:
    ...
    for grammar in grammars:
        ...
        try:
            ...
            errors[grammar.version] = InvalidInput(f"Cannot parse{tv_str}: {lineno}:{column}: {faulty_line}")

        except TokenError as te:
            # In edge cases these are raised; and typically don't have a "faulty_line".
            lineno, column = te.args[1]
            errors[grammar.version] = InvalidInput(f"Cannot parse{tv_str}: {lineno}:{column}: {te.args[0]}")
